#!/usr/bin/env bash
# Chaos - Slow SQL (v2 JOIN enrichment)
#
# 支持的服务端口：
...
curl -X POST "$CATALOG/internal/chaos/slow-sql/enable" \
  -H "Content-Type: application/json" \
  -d '{"joinTable":"product_price_history","durationSec":30}'
...
curl -X POST "$INVENTORY/internal/chaos/slow-sql/enable" \
  -H "Content-Type: application/json" \
  -d '{"joinTable":"product_price_history","durationSec":60}'
...
curl -X POST "$PAYMENT/internal/chaos/slow-sql/enable" \
  -H "Content-Type: application/json" \
  -d '{"joinTable":"user_behavior_log","durationSec":60}'
