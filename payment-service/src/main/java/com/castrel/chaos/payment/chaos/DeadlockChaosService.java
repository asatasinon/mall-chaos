...
public class DeadlockChaosService {
    ...
    private final ScheduledExecutorService scheduler = Executors.newScheduledThreadPool(3, r -> {
        Thread t = new Thread(r, "deadlock-chaos-payment");
        t.setDaemon(true);
        return t;
    });
    ...
    @PostConstruct
    void init() {
        deadlockCounter = Counter.builder("chaos.deadlock.count").tag("service", "payment")
    ...
    private void txA() {
        try {
            ...
            jdbcTemplate.execute("SELECT id FROM payments WHERE id = 1 FOR UPDATE");
            Thread.sleep(50);
            jdbcTemplate.execute("SELECT id FROM payments WHERE id = 2 FOR UPDATE");
