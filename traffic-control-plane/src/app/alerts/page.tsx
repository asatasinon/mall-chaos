...
export default function AlertsPage() {
  ...
  const [createMode, setCreateMode] = useState<'rule' | 'alertmanager'>('rule');
  ...
  const importYaml = async () => {
    ...
    setSaving(true); setError(''); setNotice('');
    try {
      const response = await fetch('/internal/alerts/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ version: config.version, yaml: sourceYaml }) });
      const result = await response.json();
      if (!response.ok || result.code !== 0) throw new Error(result.message);
      setConfig(result.data); setSourceYaml(''); setCreateModalOpen(false); setReceiverModalOpen(false); setNotice('Alertmanager YAML 已解析并应用');
    } catch (cause) { setError(cause instanceof TypeError && cause.message.toLowerCase().includes('fetch') ? '控制台接口不可达，无法解析并应用 YAML' : cause instanceof Error ? cause.message : '导入失败'); }
    finally { setSaving(false); }
  };
  ...
  return (
    <div className="space-y-5 max-w-7xl">
      ...
      {createModalOpen && <CreateAlertModal mode={createMode} onModeChange={setCreateMode} rule={draftRule} onRuleChange={setDraftRule} sourceYaml={sourceYaml} onSourceYamlChange={setSourceYaml} saving={saving} onClose={() => setCreateModalOpen(false)} onAddRule={addRule} onImportYaml={() => void importYaml()} />}
      {receiverModalOpen && <ReceiverModal receiver={draftReceiver} onChange={setDraftReceiver} sourceYaml={sourceYaml} onSourceYamlChange={setSourceYaml} saving={saving} onClose={() => setReceiverModalOpen(false)} onAdd={addReceiver} onImportYaml={() => void importYaml()} />}
    </div>
  );
}
...
function CreateAlertModal({ mode, onModeChange, rule, onRuleChange, sourceYaml, onSourceYamlChange, saving, onClose, onAddRule, onImportYaml }: { mode: 'rule' | 'alertmanager'; onModeChange: (mode: 'rule' | 'alertmanager') => void; rule: Rule; onRuleChange: (rule: Rule) => void; sourceYaml: string; onSourceYamlChange: (yaml: string) => void; saving: boolean; onClose: () => void; onAddRule: () => void; onImportYaml: () => void }) {
...
function ReceiverModal({ receiver, onChange, sourceYaml, onSourceYamlChange, saving, onClose, onAdd, onImportYaml }: { receiver: Receiver; onChange: (receiver: Receiver) => void; sourceYaml: string; onSourceYamlChange: (yaml: string) => void; saving: boolean; onClose: () => void; onAdd: () => void; onImportYaml: () => void }) {
