...
export default function AlertsPage() {
  ...
  if (!config) return <div className="space-y-4"><p className="text-sm text-muted-foreground">正在加载配置…</p>{error && <ErrorBox text={error} />}</div>;
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col gap-5">
      ...
      {activeTab === 'rules' && <section className="space-y-3">
        <div><h2 className="text-base font-semibold">告警规则</h2><p className="text-xs text-muted-foreground">PromQL 规则会生成到 Prometheus rule groups</p></div>
      ...
      {activeTab === 'routing' && <section className="space-y-3">
        ...
        <div><h2 className="text-base font-semibold">推送地址</h2><p className="text-xs text-muted-foreground">按严重级别将告警路由到 Webhook 接收器</p></div>
...
function AlertsToolbar({ activeTab, onTabChange, onRefresh, onSave, saveDisabled, saving, onAddRule, onAddRoute, onAddReceiver }: { activeTab: 'rules' | 'routing' | 'yaml'; onTabChange: (tab: 'rules' | 'routing' | 'yaml') => void; onRefresh: () => void; onSave: () => void; saveDisabled: boolean; saving: boolean; onAddRule: () => void; onAddRoute: () => void; onAddReceiver: () => void }) {
  return <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border"><div className="flex flex-wrap"><TabButton active={activeTab === 'rules'} onClick={() => onTabChange('rules')}>告警规则</TabButton><TabButton active={activeTab === 'routing'} onClick={() => onTabChange('routing')}>路由与推送地址</TabButton><TabButton active={activeTab === 'yaml'} onClick={() => onTabChange('yaml')}>整文件 YAML</TabButton></div><div className="flex flex-wrap items-center gap-2 pb-1"><Button variant="outline" size="sm" onClick={onRefresh} disabled={saving}><RefreshCw />刷新</Button><Button variant="outline" size="sm" onClick={onSave} disabled={saveDisabled}><Save />保存</Button>{activeTab === 'rules' && <Button variant="outline" size="sm" onClick={onAddRule}><Plus />新增规则</Button>}{activeTab === 'routing' && <><Button variant="outline" size="sm" onClick={onAddRoute}><Plus />新增路由</Button><Button variant="outline" size="sm" onClick={onAddReceiver}><Plus />新增地址</Button></>}</div></div>;
}
...
function YamlSourceEditor({ kind, onKindChange, yaml, loading, saving, dirty, version, onChange }: { kind: SourceKind; onKindChange: (kind: SourceKind) => void; yaml: string; loading: boolean; saving: boolean; dirty: boolean; version: number | null; onChange: (value: string) => void }) {
  return <section className="space-y-3"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-base font-semibold">整文件 YAML 编辑器</h2><p className="text-xs text-muted-foreground">直接编辑实际生效的配置文件，保存后会校验、写入并 reload 对应组件。</p></div><select className={inputClass} value={kind} onChange={(event) => onKindChange(event.target.value as SourceKind)} disabled={loading || saving}><option value="prometheus-rules">Prometheus alert-rules.yml</option><option value="alertmanager">Alertmanager alertmanager.yml</option></select></div><div className="flex items-center justify-between text-xs text-muted-foreground"><span>{loading ? '正在读取文件…' : dirty ? '有未保存修改，请使用页面右上角保存并应用' : '已与服务端同步'}</span>{version !== null && <span>配置版本 v{version}</span>}</div><textarea aria-label={`${kind} YAML source`} className="min-h-[calc(100vh-16rem)] w-full resize-y rounded-md border border-input bg-background px-3 py-3 font-mono text-xs leading-5 outline-none focus:ring-2 focus:ring-ring/40" value={yaml} onChange={(event) => onChange(event.target.value)} disabled={loading || saving} spellCheck={false} /></section>;
}
