...
| `HikariPoolExhaustion`、`HikariPoolFull`、`HikariPoolPending`、`MySQLSlowQueries`、`MySQLHighThreads` | 连接池告警达到对应规则阈值；慢查询速率超过 0.5 次/秒，持续 1 分钟；或 MySQL 连接数超过 255，持续 1 分钟 | 并发等待消耗连接或扩大到数据库层时可能出现。 |
| 无行锁等待专用告警 | 不适用 | 应将告警与终态 `SCENARIO_WORKER_STOPPED`/`SCENARIO_WORKER_DRAINED` 汇总、存在时的历史 `SCENARIO_REQUEST_FAILED`、Tempo exception/JDBC span 和 MySQL 行锁等待诊断关联。 |
