...
export default function DatePicker({ value, selectedDates, onChange, onAddDates, className = '' }: DatePickerProps) {
  ...
  const monthLabel = format.dateTime(visibleMonth, { month: 'long', year: 'numeric' });
  ...
  const rangeLabel = rangeEnd
    ? rangeDates.length === 1 ? t('daySelected') : t('daysSelected', { count: rangeDates.length })
    : rangeStart
      ? t('chooseEndDate')
      : t('selectDayToAdd');
  const displayDate = rangeEnd && rangeDates.length > 1
    ? `${formatDateLabel(parseDateValue(rangeDates[0]), format)} - ${formatDateLabel(parseDateValue(rangeDates[rangeDates.length - 1]), format)}`
    : selectedDate
  ...
  return <div ref={containerRef} className={`relative ${className}`}>
    <button type="button" className="group flex min-h-11 w-full items-center gap-3 rounded-lg border border-border bg-background/55 px-3 text-left shadow-sm outline-none transition-colors hover:border-primary/45 hover:bg-background focus-visible:ring-2 focus-visible:ring-ring" aria-haspopup="dialog" aria-expanded={open} aria-label={accessibilityT('chooseDate')} onClick={toggleOpen}>
      ...
      <span className="min-w-0 flex-1">
        <span className="block text-[10px] font-medium uppercase tracking-[0.12em] text-muted-foreground">{t('dateToAdd')}</span>
    ...
    {open && <div ref={popoverRef} className={`absolute left-0 z-40 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-border/80 bg-popover p-3 text-popover-foreground shadow-xl ${placement === 'top' ? 'bottom-[calc(100%+0.5rem)]' : 'top-[calc(100%+0.5rem)]'}`} role="dialog" aria-label={accessibilityT('chooseDate')}>
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          <p className="text-[10px] font-medium uppercase tracking-[0.12em] text-muted-foreground">{t('calendar')}</p>
          <p className="text-sm font-semibold">{monthLabel}</p>
        </div>
        <div className="flex items-center gap-1">
          <button type="button" aria-label={accessibilityT('previousMonth')} title={accessibilityT('previousMonth')} className="flex size-8 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={() => setVisibleMonth((current) => new Date(current.getFullYear(), current.getMonth() - 1, 1))}><ChevronLeft className="size-4" /></button>
          <button type="button" aria-label={accessibilityT('nextMonth')} title={accessibilityT('nextMonth')} className="flex size-8 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={() => setVisibleMonth((current) => new Date(current.getFullYear(), current.getMonth() + 1, 1))}><ChevronRight className="size-4" /></button>
        </div>
      </div>
      <div className="grid grid-cols-7 gap-1 text-center" role="grid" aria-label={monthLabel}>
        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((weekday) => <span key={weekday} className="pb-1 text-[10px] font-medium uppercase tracking-wide text-muted-foreground" role="columnheader">{format.dateTime(new Date(2024, 0, 7 + ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].indexOf(weekday)), { weekday: 'short' }).slice(0, 1)}</span>)}
        {dateCells.map((cell) => {
      ...
      <div className="mt-3 space-y-2 border-t border-border/70 pt-3">
        <div className="flex items-center justify-between gap-2">
          <button type="button" className="text-xs font-medium text-primary transition-colors hover:text-primary/75 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" onClick={() => selectDate(today)}>{t('today')}</button>
          <span className="text-[11px] text-muted-foreground">{allRangeDatesQueued ? t('alreadyQueued') : rangeLabel}</span>
        </div>
        <Button size="sm" className="w-full" onClick={addSelectedDates} disabled={rangeDates.length === 0 || allRangeDatesQueued}><Plus />{allRangeDatesQueued ? t('added') : rangeDates.length > 1 ? t('addDays', { count: rangeDates.length }) : t('addDay')}</Button>
      </div>
    </div>}
  </div>;
}
...
function formatDateLabel(date: Date | null, format: ReturnType<typeof useFormatter>): string {
  return date ? format.dateTime(date, { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric' }) : '';
}
