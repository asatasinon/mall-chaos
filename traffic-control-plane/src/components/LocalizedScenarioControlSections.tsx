...
} from '@/components/scenarios/fault-run-view';
...
import type { FaultRun, FaultRunDetails, FaultRunExecution, Scenario } from '@/components/scenarios/types';
...
export function RunHistory({ runs, scenarios, filterState, filterScenario, setFilterState, setFilterScenario, onDetails, onCleanup }: { runs: FaultRun[]; scenarios: Scenario[]; filterState: string; filterScenario: string; setFilterState: (value: string) => void; setFilterScenario: (value: string) => void; onDetails: (run: FaultRun) => Promise<void>; onCleanup: (run: FaultRun) => void | Promise<void> }) {
  ...
  return <Card><CardHeader className="flex flex-wrap items-center justify-between gap-3 space-y-0"><CardTitle className="text-sm">{t('recentRuns')}</CardTitle><div className="flex gap-2"><label className="sr-only" htmlFor="run-state-filter">{t('filterByState')}</label><select id="run-state-filter" className="h-8 rounded-md border border-input bg-background px-2 text-xs" value={filterState} onChange={(event) => setFilterState(event.target.value)}><option value="ALL">{t('allStates')}</option>{['CREATING', 'ACTIVE', 'RECOVERING', 'RECOVERED', 'STOPPED', 'FAILED', 'SERVICE_UNAVAILABLE'].map((state) => <option key={state} value={state}>{getScenarioStateLabel(state, translate)}</option>)}</select><label className="sr-only" htmlFor="run-scenario-filter">{t('filterByScenario')}</label><select id="run-scenario-filter" className="h-8 max-w-52 rounded-md border border-input bg-background px-2 text-xs" value={filterScenario} onChange={(event) => setFilterScenario(event.target.value)}><option value="ALL">{t('allScenarios')}</option>{scenarios.map((scenario) => <option key={scenario.scenario} value={scenario.scenario}>{getScenarioLabel(scenario.scenario, translate)}</option>)}</select></div></CardHeader><CardContent>{runs.length === 0 ? <p className="text-sm text-muted-foreground">{t('noMatchingRuns')}</p> : <div className="divide-y divide-border">{runs.map((run) => {
    const cleanupAvailable = isManualCleanupAvailable(run);
...
export function RunDetails({ details, onClose, onCleanup }: { details: FaultRunDetails; onClose: () => void; onCleanup?: (run: FaultRun) => void | Promise<void> }) {
  ...
  const cleanupAvailable = isManualCleanupAvailable(details.run);
  const audits = details.audits.length > 0
    ? details.audits
    : details.audit ? [details.audit] : [];
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-3 backdrop-blur-[2px] sm:items-center sm:p-6" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className="max-h-[92vh] w-full max-w-4xl overflow-auto rounded-xl border border-border bg-card shadow-[0_24px_80px_rgb(20_20_19_/_0.24)]" role="dialog" aria-modal="true" aria-labelledby="fault-run-details-title"><div className="sticky top-0 z-10 flex flex-wrap items-start justify-between gap-4 border-b border-border bg-card/95 p-5 backdrop-blur-sm"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">{scenarioT('faultRunDetails')}</p><Badge variant={stateVariant(details.run.state)}>{getScenarioStateLabel(details.run.state, scenarioTranslate)}</Badge></div><h2 id="fault-run-details-title" className="mt-2 text-xl font-semibold tracking-tight">{getScenarioLabel(details.run.scenario, scenarioTranslate)}</h2><p className="mt-1 truncate font-mono text-xs text-muted-foreground">run/{details.run.faultRunId}</p></div><div className="flex items-center gap-2"><Button variant="ghost" size="sm" onClick={onClose}>{scenarioT('details')}</Button>{cleanupAvailable && onCleanup && <Button variant="outline" size="sm" onClick={() => void onCleanup(details.run)}><Trash2 className="size-3.5" />{scenarioT('cleanupRun')}</Button>}</div></div><div className="space-y-5 p-5"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><ScenarioDetail label={scenarioT('target')} value={details.run.targetService} /><ScenarioDetail label={scenarioT('operation')} value={details.run.targetOperation} /><ScenarioDetail label={scenarioT('expires')} value={format.dateTime(new Date(details.run.expiresAt), 'compact')} /><ScenarioDetail label={scenarioT('audit')} value={details.audit?.result ? `${details.audit.result} · #${details.audit.id}` : details.run.operatorAuditId ? `#${details.run.operatorAuditId}` : scenarioT('pendingAudit')} /></div>{details.run.execution && <ExecutionOwnershipPanel execution={details.run.execution} />}{details.actions && details.actions.length > 0 && <ActionJournalPanel actions={details.actions} />}<RecoveryPanel recovery={view.recovery} />{details.run.scenario === 'CATALOG_REDIS_LARGE_VALUE' && <RunParametersPanel run={details.run} />}{view.targetSummary && <TargetSummaryPanel details={details} view={view} />}{view.workerStats && <WorkerStatsPanel stats={view.workerStats} />}{view.drain && <DrainPanel drain={view.drain} />}{view.cleanup && <CleanupPanel cleanup={view.cleanup} />}{audits.length > 0 && <AuditPanel audits={audits} />}<section aria-labelledby="fault-run-events-title" className="border-t border-border pt-5"><div className="mb-3 flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">{scenarioT('auditTrail')}</p><h3 id="fault-run-events-title" className="mt-1 text-sm font-medium">{scenarioT('eventTimeline')}</h3></div><span className="font-mono text-xs text-muted-foreground">{scenarioT('events', { count: details.events.length })}</span></div>{details.events.length === 0 ? <p className="text-sm text-muted-foreground">{scenarioT('noEvents')}</p> : <div className="space-y-2">{details.events.map((event) => <EventRow key={event.id} event={event} />)}</div>}</section></div></div></div>;
}

function ExecutionOwnershipPanel({ execution }: { execution: FaultRunExecution }) {
  const t = useTranslations('Scenarios');
  const format = useFormatter();
  const owner = execution.ownerId ?? t('noOwner');
  return <section aria-labelledby="execution-ownership-title" className="rounded-lg border border-primary/20 bg-primary/[0.035] px-4 py-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">{t('executionOwnership')}</p><h3 id="execution-ownership-title" className="mt-1 text-sm font-medium">{execution.reconciliationState}</h3></div><Badge variant="outline">{execution.mode}</Badge></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><ScenarioDetail label={t('owner')} value={owner} /><ScenarioDetail label={t('ownerEpoch')} value={String(execution.ownerEpoch)} /><ScenarioDetail label={t('heartbeat')} value={execution.lastHeartbeatAt ? format.dateTime(new Date(execution.lastHeartbeatAt), 'compact') : t('notReported')} /><ScenarioDetail label={t('leaseExpires')} value={execution.leaseExpiresAt ? format.dateTime(new Date(execution.leaseExpiresAt), 'compact') : t('notReported')} /><ScenarioDetail label={t('drainState')} value={execution.drainState} /><ScenarioDetail label={t('lastAction')} value={execution.lastAction ?? t('notReported')} /></div>{execution.ownerId && execution.takeoverCount > 0 && <p className="mt-3 rounded-md border border-amber-500/30 bg-amber-500/[0.08] px-3 py-2 text-xs text-amber-800 dark:text-amber-200">{t('ownerOverlapWarning')}</p>}</section>;
}

function ActionJournalPanel({ actions }: { actions: NonNullable<FaultRunDetails['actions']> }) {
  const t = useTranslations('Scenarios');
  const format = useFormatter();
  return <section aria-labelledby="action-journal-title" className="rounded-lg border border-border px-4 py-3"><div className="flex items-center justify-between gap-3"><h3 id="action-journal-title" className="text-sm font-medium">{t('actionJournal')}</h3><span className="text-xs text-muted-foreground">{actions.length}</span></div><div className="mt-3 space-y-2">{actions.map((action) => <div key={action.actionId} className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-border/70 bg-background/30 px-3 py-2.5"><div><p className="text-xs font-semibold">{action.actionType} · {action.actionState}</p><p className="mt-1 text-[11px] text-muted-foreground">{t('requestedBy')}: {action.requestedBy} · {format.dateTime(new Date(action.requestedAt), 'compact')}</p></div><Badge variant={action.actionState === 'OUTCOME_UNKNOWN' ? 'destructive' : 'outline'}>{action.attemptNo}</Badge></div>)}</div></section>;
}
...
function RecoveryPanel({ recovery }: { recovery: FaultRun['recovery'] }) {
  const t = useTranslations('Scenarios');
  const format = useFormatter();
  if (recovery.kind !== 'SAFE_RUNTIME_V1') {
    const message = recovery.kind === 'LEGACY'
      ? t('recovery.legacy')
      : t('recovery.unknown');
    return <section aria-labelledby="recovery-status-title" className="rounded-lg border border-border bg-muted/25 px-4 py-3"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">{t('recovery.label')}</p><h3 id="recovery-status-title" className="mt-1 text-sm font-medium">{message}</h3></div><Badge variant="outline">{t('recovery.unknownState')}</Badge></div></section>;
  }
  ...
  const metrics = projection.drain.metrics
    ? Object.entries(projection.drain.metrics).filter(([, value]) => value !== undefined)
    : [];
  return <section aria-labelledby="recovery-status-title" className="rounded-lg border border-primary/20 bg-primary/[0.035] px-4 py-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.14em] text-primary">{t('recovery.label')}</p><h3 id="recovery-status-title" className="mt-1 text-sm font-medium">{t(`recovery.phases.${projection.phase}` as never)}</h3></div><Badge variant={projection.outcome === 'PENDING' ? 'outline' : 'destructive'}>{t(`recovery.outcomes.${projection.outcome}` as never)}</Badge></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><ScenarioDetail label={t('recovery.stopReason')} value={t(`recovery.stopReasons.${projection.stop.reason}` as never)} /><ScenarioDetail label={t('recovery.drainDeadline')} value={format.dateTime(new Date(projection.deadlines.drainAt), 'compact')} /><ScenarioDetail label={t('recovery.recoveryDeadline')} value={format.dateTime(new Date(projection.deadlines.recoveryAt), 'compact')} /><ScenarioDetail label={t('recovery.attempt')} value={String(projection.stop.attempt)} /></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{steps.map(([name, step]) => <div key={name} className="rounded-md border border-border/70 bg-background/45 px-3 py-2.5"><p className="text-[11px] uppercase tracking-[0.12em] text-muted-foreground">{t(`recovery.steps.${name}` as never)}</p><p className="mt-1 text-sm font-medium">{t(`recovery.stepStatuses.${step.status}` as never)}</p>{step.errorCode && <p className="mt-1 font-mono text-[11px] text-destructive">{t(`recovery.errorCodes.${step.errorCode}` as never)}</p>}</div>)}</div>{metrics.length > 0 && <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 rounded-md bg-muted/35 px-3 py-2 text-xs text-muted-foreground">{metrics.map(([name, value]) => <span key={name}>{t(`recovery.metrics.${name}` as never)} <strong className="font-medium text-foreground">{value}</strong></span>)}</div>}{projection.lastError && <p className="mt-3 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">{t(`recovery.errorCodes.${projection.lastError.code}` as never)}{projection.lastError.retryable ? ` · ${t('recovery.retryable')}` : ''}</p>}{projection.residuals.length > 0 && <div className="mt-3 rounded-md border border-amber-500/30 bg-amber-500/[0.08] px-3 py-2.5"><p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-amber-800 dark:text-amber-200">{t('recovery.residualsLabel')}</p><div className="mt-2 space-y-1.5">{projection.residuals.map((residual) => <p key={residual.kind} className="text-xs text-muted-foreground"><strong className="font-medium text-foreground">{t(`recovery.residuals.${residual.kind}` as never)}</strong> · {t(`recovery.responsibilities.${residual.responsibility}` as never)} · {t(`recovery.nextActions.${residual.nextAction}` as never)}</p>)}</div></div>}</section>;
}

function AuditPanel({ audits }: { audits: FaultRunDetails['audits'] }) {
  const t = useTranslations('Scenarios');
  const format = useFormatter();
  return <section aria-labelledby="operator-audit-title" className="space-y-3 rounded-lg border border-border px-4 py-3"><h3 id="operator-audit-title" className="text-sm font-medium">{t('operatorAudit')}</h3>{audits.map((audit) => <div key={audit.id} className="grid gap-3 border-t border-border/70 pt-3 first:border-t-0 first:pt-0 sm:grid-cols-4"><ScenarioDetail label={t('operatorAudit')} value={`#${audit.id}`} /><ScenarioDetail label={t('result')} value={audit.result} /><ScenarioDetail label={t('action')} value={audit.action} /><ScenarioDetail label={t('recorded')} value={format.dateTime(new Date(audit.createdAt), 'compact')} /></div>)}</section>;
}
