...
export const RUNBOOK_METADATA: Readonly<Record<FaultRunScenario, ScenarioRunbookMetadata>> = {
  ...
  INVENTORY_TABLE_EXCLUSIVE: {
    ...
    businessPath: ['LOCK TABLES inventories WRITE', 'GET /internal/inventory/availability/report', 'inventory-service'],
    tempo: tempo(
      [target('inventory-service', '/internal/inventory/availability/report')],
      ['LOCK TABLES inventories WRITE', 'GET /internal/inventory/availability/report', 'inventory-service'],
      '1s',
      ['HTTP', 'JDBC', 'EXCEPTION'],
    ),
  },
  INVENTORY_ROW_LOCK: {
    ...
    businessPath: ['SELECT ... WHERE sku = \'SKU-001\' FOR UPDATE', 'GET /internal/inventory/reservations/summary', 'inventory-service'],
    tempo: tempo(
      [target('inventory-service', '/internal/inventory/reservations/summary')],
      ['SELECT ... WHERE sku = \'SKU-001\' FOR UPDATE', 'GET /internal/inventory/reservations/summary', 'inventory-service'],
      '1s',
      ['HTTP', 'JDBC', 'EXCEPTION'],
    ),
  },
