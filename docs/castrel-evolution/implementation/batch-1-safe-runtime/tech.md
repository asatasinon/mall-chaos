...
> 状态：技术设计 v1.15，P1-02-D 至 P1-02-G、严格 runtime parser、Docker Compose Web/Worker 静态配对、单 Worker/grace 门禁、无破坏性回退文档、已部署代码 `31bc400` 的 Docker Compose 单 Worker 只读发布门禁及 fresh/historical MySQL compatibility 验证已实施；P1-11-1 启用前复核已完成，主环境 `BROWSE_REPORT_SQL`、隔离 `BROWSE_SURGE` 手工停止、隔离 `CART_CATALOG_DEPENDENCY` dispatch/drain、Report/Runner 到期停止、drain timeout、target unavailable、Worker failure、SIGTERM/restart、manual cleanup、non-releasing、单 Run 前后正常后台/消费者隔离、retention 直接运行及 safe-runtime 回退保护已记录；历史 Operator 参数 read-model 修复已完成远端 list/detail 复验。旧路径接管未完成 `safe-runtime.v1` Run 的问题已通过 legacy recovery 保护性跳过修复，并在 Docker disposable canary 上复验；MySQL TCP healthcheck readiness race 已由 Compose 修正并在远端 `19f9046` 的 fresh `p1-canary-14` 冷启动中复验；2026-09-20 14:48 只读复核确认主库 `runner_profile.enabled=1/version=3`、customer lifecycle 正在运行、补给为 `COMPLETED` 且 warmup 正在 `APPENDING`，并于 15:33 在 fresh Docker canary 取得 Report/Runner participant、drain、release 和 `VERIFY_UNAVAILABLE` 到期证据，16:06 补齐 p1-canary-19 单 Run 隔离和 retention helper 直接执行证据；verification adapter 未配置仍限制终态结论；Kubernetes 验证延期
...
- 2026-09-20 在保留的 disposable `p1-canary-12` 中直接执行生产 `runFaultRunRetention()`，插入的七天前 `RECOVERED` fixture 与关联 event 均由 `1` 变为 `0`，既有未解决 `RECOVERING` Run `ead6bc66-264b-4724-825a-fc6d07b634cd` 保留，retention Redis lock 已释放，Worker 保持 `running/0/0`。helper 因数据库/Redis client 连接保持打开，完成查询后按具体 PID 终止，退出码 `143` 只反映临时 helper 清理，不代表 retention 数据操作失败；未修改主环境或伪造恢复终态。
...
- 2026-09-20 较早的主 Compose 只读快照曾显示 `runner_profile.enabled=0`，该快照已由同日 14:48 的复核更新；不得把早期部署后的 `running=false` 解释为当前 Runner 状态。safe-runtime Web/Worker 环境始终配对且均为 `false`；此前 `BROWSE_SURGE` Run 的 `RECOVERING` 已在旧路径接管后变为 `FAILED/RECOVERY_FAILED/WORKER_DRAIN_INCOMPLETE`，不能把 active guard 消失解释为恢复成功。隔离 `p1-canary-12/13` 的正常 traffic/lifecycle、补给、消费者响应、目标服务日志和 Operator UI/i18n 边界未发现恢复上下文泄露；其 warmup 为 disabled，只能证明 stop 未制造 warmup failure。
- 2026-09-20 14:48 的远端只读复核更新了上述运行时快照：主库 `runner_profile` 为 `enabled=1`、`traffic_mode=CUSTOMER_LIFECYCLE`、`lifecycle_interval_sec=10`、`version=3`，存在运行中的 customer lifecycle traffic run；最近 1 小时 `traffic_actions` 为 `SUCCESS=242`、`NOOP=27`、`FAILED=35`，补给最近运行均为 `COMPLETED`，两张 warmup 表为 `APPENDING` 且各达到 `54,000,000` 目标。该事实纠正此前 `enabled=0` 的旧快照；随后 `p1-canary-19` 补齐了单 Run stop 前后隔离，下一条 retention 证据补齐了生产 helper 的直接执行边界。`FAILED=35` 仍未归因，不能将其折叠为成功率或安全停止结论。
- 两套历史隔离 canary Worker 最终状态均为 running、exit 0、RestartCount 0，但启动日志分别出现 `5`/`1` 次稳定码 `WORKER_STARTUP_FAILED`。时间线显示旧 `mysqladmin ping -h localhost` 在 MySQL 仅有本地 socket、网络 TCP `3306` 尚未 ready 时提前通过；后续 Redis `ECONNREFUSED`/`EAI_AGAIN` 出现在 Worker 已成功启动之后，不作为本次 startup failure 根因。Compose 已改为 `127.0.0.1:3306` TCP `SELECT 1` 并随远端 `19f9046` 部署；fresh `p1-canary-14` 冷启动中 MySQL 于 `06:36:11.979Z` 报告 TCP ready，Worker 于 `06:36:18.098Z` 启动，`WORKER_STARTUP_FAILED=0`、`RestartCount=0`、`fault_runs=0`，复验后 disposable 资源已清理。
...
- `p1-canary-15` 在 fresh MySQL volume、Runner disabled 的隔离 Docker Compose 中创建 `BROWSE_REPORT_SQL` 30 秒到期 Run `bf1a904f-59c3-4f44-86b1-f0fc4f272107`。到期时间线按 `REPORT_WORKER_STARTED -> REPORT_WORKER_STOPPED -> STOP_REQUESTED(reason=EXPIRED) -> DRAIN_COMPLETED -> RELEASE_COMPLETED -> VERIFY_UNAVAILABLE -> RECOVERY_BLOCKED` 收敛；drain projection 为 `participants=1/accepted=1/completed=1/inFlightAtFinish=0`，最终顶层 Run 仍为 `RECOVERING`，`stop_reason=EXPIRED`，没有把 release acknowledgement 当成业务验证成功。
- `p1-canary-18` 在 Worker 启动前写入 `runner_profile.enabled=1`、`lifecycle_interval_sec=5`、`version=2`，随后创建 Runner-enabled 的 `PSP_PROVIDER_OUTCOME` 45 秒到期 Run `950cde37-3270-4261-a0db-9a9436cea34d`。Runner 先产生 `RUNNER_LIFECYCLE_SUMMARY=5`，到期后时间线包含 `STOP_REQUESTED(reason=EXPIRED)`、`DRAIN_COMPLETED`、`RELEASE_COMPLETED`、`VERIFY_UNAVAILABLE`、`RECOVERY_BLOCKED`；drain projection 为 `participants=5/accepted=5/completed=4/aborted=1/inFlightAtFinish=0`，最终保持 `RECOVERING`。这证明 Runner 受控 participant 只在真实 lifecycle tick 后注册，未配置 verification 仍不能写入成功终态。
- `p1-canary-16` 的 Runner disabled 和 `p1-canary-17` 的首次 lifecycle tick 尚未到达时均产生 `DRAIN_PARTICIPANT_MISSING`，不能作为 Runner drain 成功证据。更新 `runner_profile.lifecycle_interval_sec` 不会重新调度已经创建的 timer；正确的 canary 编排是在 Worker 启动前写入 Runner 配置，或重启 Worker 后等待首次 tick，再执行到期/手工停止。新增 canary 的容器、volume、network、临时 Compose 和响应文件均已精确清理，主 Compose 数据未修改。

- `p1-canary-19` 使用独立 control-plane MySQL/Redis/Web/Worker 与共享 Gateway/业务网络，基线为 `traffic_actions=801`、warmup 两张表 `APPENDING 4/4`、补给记录 `2`、Shopfront/Gateway health HTTP `200`。创建 `PSP_PROVIDER_OUTCOME` Run `83df6a42-2734-43ee-937f-78f674f11f77`（60 秒、`providerOutcome=TIMEOUT`、`effectPercentage=100`）后手工 stop 返回 `202`；事件包含 `STOP_REQUESTED`、`DRAIN_COMPLETED`（`participants=1/accepted=1/completed=1/inFlightAtFinish=0`）、`RELEASE_COMPLETED`、`CLEANUP_SKIPPED`、`VERIFY_UNAVAILABLE`、`RECOVERY_BLOCKED`，最终保持 `RECOVERING`。停止后 Shopfront/Gateway 仍为 HTTP `200`，Runner 仍 `running=true`、`traffic_run` 仍为 `RUNNING`，新增 `75` 条 lifecycle action（`SUCCESS=58/NOOP=5/FAILED=12`），warmup 仍为 `APPENDING 4/4`；inventory/coupon replenishment 均 HTTP `200`/`COMPLETED`，补给记录增至 `4`；Gateway、PSP、Catalog、Shopfront 和目标服务日志未发现该 Run ID、scenario、`faultRunId`、`recovery_result` 或内部恢复字段。该 canary 证明单 Run stop 不调用全局 Runner stop，也不阻断 warmup、补给、消费者或目标业务路径；但共享 Gateway/业务服务不等同于完全独立业务数据库，warmup `4/4` 仅为最小隔离目标，且 `traffic_actions` 的 `FAILED=12` 未被折叠为成功。
...
- MySQL 健康检查必须验证 `127.0.0.1:3306` TCP 上的只读查询，而不能只使用可能在网络端口 ready 前成功的 `mysqladmin ping -h localhost` socket 检查；该修正已随远端 `19f9046` 部署，并以 fresh `p1-canary-14` 冷启动复验；
...
3. 已使用 fresh MySQL 与含历史 Run 的已有 volume 验证 JSON/NULL 兼容、七天 retention 和未解决 `RECOVERING` Run 不被删除；另在 `p1-canary-12` 直接执行生产 retention helper，复核终态 fixture/event 删除、未解决 Run 保留和 lease 释放。
...
5. 已在 `p1-canary-19` 验证停止一个 Run 不会停止正常 lifecycle、数据预热、库存/优惠券补给；Shopfront/Gateway 仍健康，消费者与目标服务日志没有新增控制面恢复字段。
