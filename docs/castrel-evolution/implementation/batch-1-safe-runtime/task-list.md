...
| 项目 | 内容 |
| --- | --- |
| 状态 | 进行中；P1-01、P1-02（含原 P1-03 至 P1-08）、P1-09、P1-10 已完成代码、测试和 Docker 范围证据；P1-11 Docker-only stateful canary 正在执行。主环境首个手工停止、隔离环境 Surge 手工停止、CART dispatch/drain、到期停止、drain timeout、target unavailable、Worker failure、SIGTERM/restart、manual cleanup、non-releasing 以及正常后台/消费者隔离边界均已记录；当前 Catalog verification 未配置的 Run 保持 `RECOVERING`，历史 Operator 参数兼容修复已在本地完成但尚未部署复验，当前仅在 Docker Compose 单 Worker 范围实施和验证 |
| 版本 | 1.38 |
| 更新时间 | 2026-09-20 09:43 CST（P1-11-6 正常后台/消费者隔离复核记录；历史参数兼容部署复验仍待） |
...
- **总体状态：** 进行中（Docker-only 范围）。P1-01 已完成：本地 Docker/Compose 静态检查有效，远端 Compose 配置有效。2026-09-20 只读复核确认远端部署仍为 `1c574e6`、主 Compose 控制面和 Worker 可运行；主库快照为 `FAILED:2`、`RECOVERED:23`、`RECOVERING:1`、`STOPPED:1`，当前未收敛 Run 为 `BROWSE_SURGE`、`stop_reason=EXPIRED`、`recovery_error=VERIFY_UNAVAILABLE`，`ACTIVE_GUARD_COUNT=1`，因此不能继续把主库描述为无 active guard。主库正常 Runner、补给和 warmup 仍有近期成功/进度证据；隔离 canary 已记录正常 lifecycle、补给、消费者响应、Gateway/目标日志和 Operator UI/i18n 边界，但两个 canary Worker 日志分别出现 5/1 次 `WORKER_STARTUP_FAILED`，最终容器仍为 running/exit 0，根因尚未分类。P1-02 已完成 recovery contract、持久化命令事务、Worker-only recovery executor、真实本地 drain registry、Report/Surge/Scenario runnable admission、Runner 生命周期边界、cleanup/non-releasing policy、Operator API/UI/i18n 以及当前 Catalog 的 CART Docker dispatch/drain 证据。P1-09 已完成 strict config、Compose Web/Worker pairing、singleton/grace guard、低敏恢复日志、无破坏性回退文档及当前代码的只读运行时门禁；P1-10 已完成代码、route、历史/fresh volume、retention 和静态边界覆盖。safe-runtime 开关在回退前不得默认开启。不执行 Kubernetes 验证。
- **总体进度：** 1 / 11 个任务组（59 / 65 个子任务完成；P1-02、P1-10 已完成，P1-11-1、P1-11-3、P1-11-4、P1-11-5 已完成，P1-11-2 部分完成，P1-11-6 部分完成，P1-11-7 继续进行）。
- **当前任务：** P1-11：Docker Compose 单 Worker stateful canary、验收与阶段退出；隔离环境的正常后台/消费者边界已有只读证据，但主环境 unresolved `RECOVERING` active guard、历史 Operator read-model 的远端部署复验和 canary Worker 启动异常仍未解除。
- **当前问题：** P1-ISSUE-001、P1-ISSUE-002、P1-ISSUE-003、P1-ISSUE-005 至 P1-ISSUE-008、P1-ISSUE-010 的范围/语义/实现项已解决；P1-ISSUE-004 仍受 verification adapter 未配置限制，P1-ISSUE-009 记录 cleanup 的跨进程 at-least-once 限制，P1-ISSUE-011 记录远端发布主机缺少 Node 导致 guard 脚本不能原样执行，P1-ISSUE-012 记录历史旧参数的本地兼容修复尚未在远端部署复验，P1-ISSUE-013 记录主环境 `RECOVERING`/active guard 边界，P1-ISSUE-014 记录隔离 canary `WORKER_STARTUP_FAILED` 根因未分类。这些均不得被视为完整安全停止已具备。
- **下一步：** 不删除 Run、事件、volume 或强制写入终态。先由部署流程发布包含本地历史参数 read-model 修复的版本，再做远端全量 Operator list/detail 只读复验；同时分类 P1-ISSUE-014，完成后才进入无破坏性回退判断。保持单 Worker，不执行 Kubernetes 验证。
...
**状态：** 进行中（已获授权，启用前复核）；**进度：** 0 / 7；**关联问题：** P1-ISSUE-001、P1-ISSUE-002、P1-ISSUE-003、P1-ISSUE-004
...
- [-] 核验停止单个 Run 不影响正常 customer lifecycle、数据预热、补给或 retention；检查消费者响应、Gateway 请求、目标服务日志、Operator UI 和 i18n 没有泄露内部恢复上下文、凭据或 raw stack。隔离 `p1-canary-12/13` 已有正常 lifecycle、补给、消费者响应和目标日志无敏感字段的只读证据；主 Compose Runner 正常运行、补给最近完成、warmup 已达到目标行数并持续更新，`DATA_WARMUP_ENABLED=false` 的隔离 canary 只能证明未被 stop 破坏，不能证明隔离 warmup 写入持续。retention 仅有代码/测试和既有负向 failure counter 证据；另有 `WORKER_STARTUP_FAILED` 根因未分类，故本项仍为部分完成。
...
| P1-ISSUE-012 | P1-11-1：启用前复核 | 当前历史 volume 中有两条旧版 `CATALOG_REDIS_LARGE_VALUE` Run 使用 `memberSizeBytes=256`；当前 Catalog 下限为 `1024`，Operator read model 对每条 Run 重新执行严格参数校验，导致未过滤 `GET /internal/fault-runs` 返回 `400 INVALID_PARAMETER:memberSizeBytes`。 | Operator 列表和依赖全量列表的 UI 不能读取完整历史 Run；不能把旧参数当作当前 Catalog 合法值，也不能通过修改现有数据掩盖问题。 | 保留历史 Run 和原始参数不变；本地工作树已在 `fault-run-operator-view` 增加 allowlist/类型安全的历史参数保留、`parameterStatus=LEGACY|UNKNOWN` 和稳定 `parameterIssue`，并更新 Operator UI/i18n；`pnpm test:runner`（196 passed）、`pnpm test:i18n`（16 passed）、typecheck、lint、build、Compose guard、runtime terminology 和 diff check 均已通过。远端仍为 `1c574e6`，未包含本地 `f96b9ef` 之后的未提交兼容修复，因此尚未完成全量 Operator list/detail 的部署复验。 | 处理中（本地修复/测试完成；远端重部署复验待执行） |
| P1-ISSUE-013 | P1-11-2：首个手工停止 canary | 首个真实 `BROWSE_REPORT_SQL` stop 已完成 drain 和 target release，但当前 Catalog verification 为 `NOT_CONFIGURED`，Worker 正确写入 `VERIFY_UNAVAILABLE`/`PARTIAL_RECOVERY` 并保留顶层 `RECOVERING`。2026-09-20 主 Compose 只读快照仍有 `BROWSE_SURGE`、`stop_reason=EXPIRED`、`recovery_error=VERIFY_UNAVAILABLE` 的 `RECOVERING` Run，`ACTIVE_GUARD_COUNT=1`；隔离 `BROWSE_SURGE` stop 也在不需要 target release 的 policy 下保持相同 unresolved 边界。 | 不能把 control-plane stop acknowledgement、release completion、release skip 或 health 200 当作恢复成功；同一持久 MySQL 环境无法继续创建其它 Run，否则会违反单 active Run 和 unresolved recovery 的安全边界。 | 不强制终态、不删除 Run/事件/volume。隔离 canary 已使用独立 control-plane 数据库保留各自的投影、事件和审计，并完成正常后台/消费者隔离读取；主环境仍保持 unresolved guard，待真实 verification adapter 或明确回退窗口处理。 | 已阻塞（主环境；隔离 canary 可继续） |
