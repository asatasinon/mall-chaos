...
> 下一步：AgentRcaReport v1 Schema 实现和 pilot receiver 配置
...
| Operator | Agent 输出格式不一致，无法进入统一评估 | 获得标准化 AgentRcaReport v1 |
...
- 接收 `schemaVersion = agent-rca-report.v1` 的结构化报告。
- 使用 `reportId` 做幂等和审计。
...
Alertmanager 发送 pilot firing alert
  ...
  -> Agent 生成 AgentRcaReport v1
  -> 控制面校验并按 reportId 幂等接收
  -> 合法提交进入 Evaluator 队列
...
| 结果 | 含义 |
| --- | --- |
| `ACCEPTED` | 格式、告警引用和幂等校验通过，进入 Evaluator；Fault Run 关联状态作为评估限制单独记录 |
| `REJECTED` | 缺少必需字段、告警引用不存在或包含禁止内容 |
| `DUPLICATE` | 相同 reportId 已处理，返回原处理事实 |
| `EVALUATION_CLOSED` | 服务端或 Operator 已显式关闭该告警评估 |

观测 retention 不足时，报告仍可接收；后续评估应记录 `EVIDENCE_UNAVAILABLE`，不能直接判定 RCA 错误。

一个 `alertRef` 只代表一个告警实例；同一问题产生多个告警时，AgentRcaReport 使用 `alertRefs[]` 一次引用多个实例。Alertmanager 的 `groupKey` 只是通知分组，不能直接证明这些告警属于同一根因；是否形成 `incidentRef` 由控制面根据告警合同、时间窗口、服务/资源关联和 active Fault Run 事实判断。

`incidentRef` 是控制面内部的聚合引用，不要求 Agent 生成，也不作为 `AgentRcaReport.json` 的必填字段；服务端根据 `alertRefs[]` 在接收和评估时解析或确认它。
...
- Agent 可以完成只读观测查询并提交合法 `AgentRcaReport v1`。
- 非法、重复和评估关闭后的提交均有明确结果。
- Alertmanager 已投递但没有唯一 Fault Run 上下文的提交仍返回 `ACCEPTED`，并在评估报告中记录 `faultRunCorrelationStatus` 限制，不被伪装成 `REJECTED`。
- Agent 无法通过提交内容获得 Fault Run 控制字段或内部权限。
- AgentRcaReport 可以覆盖一个问题对应的多个告警实例，并能区分主要告警和补充告警。
- AgentRcaReport 只引用部分有效告警时仍可进入 Evaluator，不因为未列出全部告警而自动拒绝。
...
本规格参考 ITBench 的**实验评估思想和资产边界**，但 `AgentRcaReport.json` 不是从 ITBench 复制的现成跨领域标准。ITBench 当前不同领域主要由 Scenario、Fault、Waiter、Ground Truth、Recorder、状态/Bundle 和外部评估流程组合而成，并不存在一个可以直接复用到 Castrel 的统一 `AgentRcaReport` 本地模型。
...
| 状态、Bundle、评估输入 | `alertRefs[]`、内部 `incidentRef`、告警接收记录、AgentRcaReport、Evaluator Report | 否，按告警驱动重新设计 |
| 外部 Agent 接入 | Alertmanager 外部 receiver + Nginx Basic Auth + Agent RCA report endpoint | 否，不引入 AWX 或额外 Agent Gateway |
