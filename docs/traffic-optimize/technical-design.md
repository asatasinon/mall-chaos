...
| `lifecycle_interval_sec` | enum | 仅允许 `60`、`30`、`20`、`10`、`5`；一次生命周期结束后等待此间隔再开始下一次。 |
...
`RunnerEngine` 主模式中每次只执行一条生命周期。生命周期完成后，按 `lifecycle_interval_sec` 等待 `60`、`30`、`20`、`10` 或 `5` 秒，再启动下一条；不再计算或展示 QPS、峰值倍率、周期和抖动。状态 API 返回当前选择间隔、上次开始/结束时间、累计完成/中断数和实际平均间隔。若单次执行时长超过选择间隔，下一次在完成后立即按最小安全延迟启动，不允许重叠执行。
...
1. 移除 QPS、峰值倍率、周期和抖动控件，改为 `60s`、`30s`、`20s`、`10s`、`5s` 串行执行间隔单选。
...
2. 验证 Gateway 健康后，分别选择 `60s`、`30s`、`20s`、`10s`、`5s` 执行间隔，确认生命周期严格串行且记录实际间隔。
