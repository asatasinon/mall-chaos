...
| 阶段 | 目标 | 状态 | 进度 | 前置依赖 |
| --- | --- | --- | --- |
| A | Fault Run 契约、持久化与单运行协调 | 待开始 | 0 / 5 | 无 |
...
- [ ] 验证两个表均为 `RANGE COLUMNS` 日分区且所有唯一键符合 MySQL 分区限制；初始化后存在 180 个东八区分区、每分区 500,000 行、总量约 90M。
...
4. 两张慢 SQL 数据表以东八区日分区维持 180 天、每日 50 万、每表约 90M 行窗口，日切以创建/删除分区完成。
