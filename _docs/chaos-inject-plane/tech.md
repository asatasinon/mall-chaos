...
Promotion 新增受认证的内部“优惠券预留一致性核对”接口。该接口只作用于运行创建的、可识别的过期预留记录，并启动两条真实事务：优惠券预留路径按 `coupon -> coupon_reservation` 获取锁，过期核对路径按 `coupon_reservation -> coupon` 获取锁。MySQL 死锁产生的原始 `SQLException` 直接从服务接口抛出；Gateway 将下游 5xx 统一转换为失败 `ApiResponse`，服务日志和 trace 保留完整异常证据。事务由连接关闭回滚，停止或到期时再删除准备记录，不能污染客户真实优惠券。
