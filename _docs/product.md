...
- 结算以服务端活动购物车快照为准：客户提交 `cartId`、`cartVersion`、地址、优惠券和幂等键，不提交权威商品明细或金额。结算期间可使用 Redis 保存带有效期的冻结快照和互斥令牌；订单明细仍必须持久化到 MySQL，不能依赖 Redis 作为订单事实来源。
- 客户结算必须携带幂等键；重复提交返回同一订单，不能重复预占库存。结算成功后，只消费与该购物车版本匹配的明细，避免并发加购被清空。

### 4.4 模拟支付、订单和物流

- 结算后展示已创建支付意图的支付页，客户确认模拟支付并得到成功、失败或超时结果。
- 支付尝试与订单状态分离：明确不可重试的 `FAILED`、客户取消和预占到期时释放库存；`TIMEOUT` / `UNKNOWN` 必须先对账，不立即释放。
- 订单可重试支付时保留有效库存预占；预占过期后的重试必须先原子重新预占所有 SKU 和优惠券，失败则不能支付。
- 客户可查看分页订单历史和订单详情，详情包含明细、冻结金额、支付状态、地址快照和履约状态。
- 仅未支付、未发货订单可取消；取消释放库存。
- 已支付订单同步完成支付后风控、履约和通知；仅风控通过后创建演示发货单和物流时间线。客户收到支付、风控和履约结果的站内消息或模拟邮件通知。
...
- runner 不模拟浏览器会话，不使用客户 Cookie，也不将任意 `userId` 作为公开 API 参数；它在控制面进程内编排，并使用专用服务凭据经网关调用与客户相同的公开 API。
- 环境重置由运维手工执行：先停止所有业务服务、Gateway、runner 和业务流量，再清除 MySQL 与 Redis 数据，最后重新启动并完成初始化和健康检查后恢复 runner。
...
| 库存 | 有效结算的每个明细均以 `reservationId` 预占；不可重试失败、取消或到期时释放全部预占；超时先对账。 |
...
| 并发裁决 | 支付成功、取消和预占到期只能有一个赢得订单终态转换；其余操作读取最终状态并执行规定补偿。 |
...
`PAYMENT_FAILED` 仅表示本次支付尝试失败。订单只有在预占仍有效、未取消且失败可重试时才能回到 `PENDING_PAYMENT`；`TIMEOUT` / `UNKNOWN` 先对账，不能与确定失败混用。
...
10. 支付成功、取消和预占到期并发时，最终只有一种订单终态；库存和优惠券不会同时确认与释放。
11. 运维先停止全部服务和业务流量，再清除 MySQL/Redis、初始化、健康检查并恢复 runner；重置后不存在旧订单队列或幂等键干扰。
...
交付顺序：身份/授权、全新数据库 Schema 和测试基础；商品读模型和持久化购物车；多商品结算、支付和订单 API；同步履约和通知；独立消费者前台、部署边界和端到端验证。
