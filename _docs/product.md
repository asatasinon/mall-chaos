...
- 结算以服务端活动购物车快照为准：客户提交 `cartId`、`cartVersion`、地址、优惠券和幂等键，不提交权威商品明细或金额。
...
- 数据库重置时，必须停止 runner 与业务流量，并同时清除 MySQL 和 Redis 中引用业务数据的状态；完成 Schema、种子数据和健康检查后，才能恢复 runner。
...
11. 清库重建按停流、清除 MySQL/Redis、初始化、健康检查、恢复 runner 的顺序执行，重置后不存在旧订单队列或幂等键干扰。
