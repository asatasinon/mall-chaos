...
| 维度 | v1 | v2 |
|------|----|----|
| 代码命名 | `ChaosController`、`SlowSqlChaosService`、`chaos_switch` 表 | 正常业务命名，无 chaos 字样 |
| 慢 SQL 实现 | `SELECT SLEEP(N)` / `Thread.sleep()` | JOIN 3000 万行大表，真实全表扫描 |
| 死锁实现 | 构造两个事务交叉锁行 | `LOCK TABLES` 锁整张表，后续流量自然阻塞超时 |
| 内存泄漏实现 | 独立线程分配 `byte[]` | 业务请求驱动，查询结果缓存不释放 |
| 触发方式 | Chaos Profile + REST API | Redis 开关 + 正常业务 API |
| Spring Profile | `chaos` profile 隔离 | 无需特殊 profile，始终存在 |
| 优化空间 | 关闭注入即恢复 | 需要真正的 SQL 优化 / 索引调整 / 缓存修复 |
