...
| Phase 6 | Shopfront、部署与端到端验收 | 进行中 | 3 / 5 | Phase 1 至 Phase 5 |
...
## Phase 6：Shopfront、部署与端到端验收 - 进行中

**阶段进度**：3 / 5

**目标**：交付独立消费者前台、部署隔离和完整业务/混沌验收。本阶段按请求跳过认证 UI 和客户令牌签发；Shopfront 只转发已有认证凭据或可选的服务端演示凭据。

### 子任务

- [x] T6.1 新建独立 `shopfront` Next.js 应用及 BFF：实现已有凭据转发、强类型网关客户端、商品列表/详情、购物车、结算、支付结果、账户资料、地址、订单、物流和通知路由；按本阶段范围跳过认证 UI 和客户令牌签发。
...
- [-] T6.5 执行 Phase 6 发布验收：注册至订单签收完成、资料/默认地址、通知已读、退款越权/幂等、指标/链路/脱敏、部署隔离和恢复流程；认证注册/登录按本阶段请求跳过，其余全栈发布验收仍待部署环境。
...
| P6-002 | 2026-08-24 | 当前本地没有完整 Gateway、业务服务和有效演示凭据，无法宣称从结算到签收、Outbox/Inbox、指标链路和混沌恢复已完成发布验收。 | T6.4、T6.5 | 保留实现任务与配置验证结果；待完整 Compose 或部署服务器可用后执行跨服务验收，不将认证缺失误报为故障。 | 未关闭 |
*** End Patch