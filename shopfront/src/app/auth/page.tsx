...
export default function AuthPage() {
  ...
  return <>
    ...
    {session ? <div className="auth-layout reveal">
      <section className="surface session-panel"><span className="session-mark"><Check size={19} /></span><span className="eyebrow">session active</span><h2>Welcome back.</h2><p>This browser is signed in as customer <strong>#{session.userId}</strong>. Your session token stays in an HttpOnly cookie and is never exposed to page JavaScript.</p><div className="session-actions"><button className="btn light" onClick={refresh} disabled={busy}><RefreshCw size={15} /> {busy ? 'Working' : 'Refresh session'}</button><button className="btn danger" onClick={signOut} disabled={busy}><LogOut size={15} /> Sign out</button></div></section>
