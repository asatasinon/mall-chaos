...
export default function OrderDetailPage() {
  ...
  const retry = async () => { setBusy(true); setError(''); try { const payment = order?.paymentId ? await retryPayment(Number(id)) : await createPaymentIntent(Number(id), `shopfront-payment-${order?.orderNo}`); if (!payment?.id) throw new Error('支付意图创建失败'); localStorage.setItem(`shopfront-payment:${payment.id}`, JSON.stringify({ payment, order })); router.push(`/payment/${payment.id}`); } catch (e) { setError(e instanceof Error ? e.message : '支付重试暂时失败'); } finally { setBusy(false); } };
