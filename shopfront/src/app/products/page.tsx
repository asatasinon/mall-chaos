...
export default function ProductsPage() {
  ...
  return <><div className="page-heading"><div><span className="eyebrow">01 / catalog</span><h1>The current edit.</h1></div><p>Browse the available collection. Search and sorting stay deliberately simple, so the useful thing remains easy to find.</p></div>
    <form className="toolbar" onSubmit={(event) => { event.preventDefault(); setPageNumber(0); load(0); }}><Search size={17} color="var(--muted)" /><input className="input search-input" value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="Search by name or keyword" aria-label="Search products" /><select className="select" value={category} onChange={(event) => { setCategory(event.target.value); setPageNumber(0); }} aria-label="Product category"><option value="">All categories</option><option value="home">Home</option><option value="desk">Desk</option><option value="daily">Daily</option></select><select className="select" value={sort} onChange={(event) => setSort(event.target.value)} aria-label="Sort products"><option value="latest">Latest</option><option value="price_asc">Price / low</option><option value="price_desc">Price / high</option></select><button className="btn" type="submit">Filter</button></form>
