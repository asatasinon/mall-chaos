...
export default function ProductDetailPage() {
  ...
  return <><Link className="eyebrow" href="/products"><ArrowLeft size={13} /> back to catalog</Link><div className="detail-layout" style={{ marginTop: 30 }}><div className="detail-visual">{product.mediaUrl ? <img src={product.mediaUrl} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span className="product-placeholder">{product.name.slice(0, 1)}</span>}</div><div className="detail-copy"><span className="eyebrow">{product.category ?? 'collection'} / {product.sku}</span><h1>{product.name}</h1><p>A considered object from the current Castrel collection. Availability is checked again when the basket is settled.</p><span className="price">{money(product.price)}</span><div className="detail-rule" /><span className="detail-label">quantity / {product.availableQty ?? 0} available</span><div style={{ display: 'flex', alignItems: 'center', gap: 14 }}><div className="quantity"><button aria-label="减少数量" onClick={() => setQuantity(Math.max(1, quantity - 1))}>−</button><span>{quantity}</span><button aria-label="增加数量" onClick={() => setQuantity(Math.min(product.availableQty ?? 99, quantity + 1))}>+</button></div><button className="btn" disabled={!available} onClick={() => addCartItem(product.sku, quantity).then(() => { setMessage('Added to your basket.'); window.dispatchEvent(new Event('cart-updated')); }).catch((e: Error) => setError(e.message))}><Plus size={15} />{available ? 'Add to basket' : 'Unavailable'}</button></div>{message && <p style={{ color: 'var(--acid-deep)', fontWeight: 700 }}><Check size={15} /> {message}</p>}{error && <ErrorNotice message={error} />}</div></div></>;
}
