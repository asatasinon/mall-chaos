...
export default function AddressesPage() {
  ...
  useEffect(() => { load(); }, []);
  const submit = async (event: React.FormEvent) => { event.preventDefault(); try { await api<Address[]>('addresses', { method: 'POST', body: JSON.stringify(form) }); setForm(emptyForm); setOpen(false); load(); } catch (e) { setError(e instanceof Error ? e.message : '地址保存失败'); } };
  const remove = async (id: number) => { try { await api<Address[]>(`addresses/${id}`, { method: 'DELETE' }); load(); } catch (e) { setError(e instanceof Error ? e.message : '地址删除失败'); } };
